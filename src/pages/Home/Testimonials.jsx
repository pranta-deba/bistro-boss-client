...
const Testimonials = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:5000/review').then(res => res.json()).then(data => {
            setReviews(data)
        });
    }, [])
            ...
            <div>
                <Swiper navigation={true} modules={[Navigation]} className="mySwiper">
                    {reviews.map(review => <SwiperSlide key={review._id}>
                        <div className="text-center max-w-[1096px] mx-auto space-y-3">
                            ...
                            <p>{review?.details}</p>
                            <h1 className="text-[#CD9003] text-3xl font-normal uppercase">{review?.name}</h1>
                        </div>
                    </SwiperSlide>)}
                </Swiper>
            </div>
